...
filename = "./data_ray/CDL-B.mat" %path to save dataset
...
mimo_downlink = zeros(number/4, RBNum*12, 14, prod(UETX), prod(BSTX));
...
for vi = 1:4
    ...
    for index = 1:number/4
        ...
        if isempty(rays{1})
            ...
            lambda = c/fc;
            ueArray = phased.ULA('NumElements', 2,'ElementSpacing', 0.5*lambda);             
...
save(filename,'mimo_downlink','lat','lont','-v7.3')
